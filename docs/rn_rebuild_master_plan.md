...
- 当前主模块：推荐、广场、创作（替换邂逅中）、我的、角色详情、故事详情、阅读器、分支剧情图、角色聊天

### 1.2 目标项目（React Native）
- 路径：`reading-app-rn/`
- 当前状态：核心业务已大面积迁移；阅读器主链路完成，聊天页已打通流式回复，创作页（替换邂逅）进行中
...
- 关键迁移点：
  ...
  - 路由文件结构对齐：搜索页已从单文件迁移为目录页 `app/search/index.tsx`
  - 瀑布流实现说明：
...
- 关键迁移点：
  - 三标签内容（浏览记录/收藏/角色）
  - 更多邂逅引导
  - 顶部用户区（头像/昵称/签名/编辑入口）与安全区间距
  - 阅读记录卡片、角色卡片、空态与开发测试入口样式对齐
  - 角色卡点击规则：已互动角色可进入详情，未解锁角色提示文案
...
- RN Feature：
  ...
  - `src/features/chat`
- 关键迁移点：
  - 角色数据融合（真实角色 + 虚拟角色默认补全）与统一详情渲染
  - 聊天历史落库
  - AI 上下文截断（最近 N 条）
  - 角色详情入口统一走导航工具，避免页面层重复拼接路由参数

## 5.8 路由封装（RN 对齐 Flutter AppNavigator）
- Flutter 参考：
  - `utils/app_navigator.dart`
- RN Feature：
  - `src/navigation/appNavigator.ts`
- 关键迁移点：
  - 高频跳转统一封装（角色详情 / 故事详情 / 阅读器 pure&interactive / 搜索 / 邂逅）
  - 复杂参数拼接收敛（如阅读模式 query 参数）
  - 页面内仅调用导航方法，减少重复路由字符串与维护成本
...
- 步骤：
  ...
  6. 分支图面板与回滚
- 暂缓项（不阻塞后续阶段）：
  - 角色弹层卡片图片缩放/对齐精修（当前已可用，仍需进一步还原 Flutter 视觉）
  - 阅读器开场动画遮罩层迁移（低优先级，后续统一补齐动效）
- 已完成细节优化（并入 Phase 5）：
  - 分支选择模块已按 Figma `selection-area` 对齐（标题/选项/custom-input）
  - custom-input 展示逻辑已修正：仅用户自建故事（UGC）展示，固定故事 `story_001/002/003...` 不展示
  - 高光时刻标注样式已对齐（底色 `#005DFF` 4%）
  - 高光背景图展示已修复：对齐 Flutter `cover + center` 视觉效果，避免仅显示左上局部
- 验收：
  - 完成一次完整阅读链路（含分支选择与回退）
...
- 状态：`doing`
- 目标：
  1. 以 Tab 页承载“用户创作”，替换原邂逅入口能力
  2. 自由模式 / 固定结局模式 UI 与交互对齐设计稿
  3. 所有文案保持中英文双语
- 当前进展（已完成）：
  - `app/(tabs)/encounter.tsx` 已重构为创作页首版（自由/固定结局双模式）
  - 已完成灵感设定、题材选择、玩法模板（自由模式）、结局目标（固定模式）与生成按钮校验链路
  - 已接入“更多题材”弹层、模式选择下拉层、PC 提示弹层，并按设计稿持续细化样式
  - 已补齐创作页中英文文案资源（`create.*`）
  - 已修复固定结局模式下“结局目标为空”生成校验的红色边框反馈
  - 已修复自由模式下“当前模式”文案异常（避免 i18n key 直出）
- 当前待优化（持续进行）：
  - 继续做创作页像素级对齐（题材卡片、按钮与间距等细节）
  - “生成故事”仍为占位通知，待后续接入真实 API
- 验收：
  - 创作主流程可稳定走通（模式切换 -> 填写/选择 -> 校验 -> 提示）
  - 中英文切换下不出现 key 直出或缺失文案
...
- 阅读器内容量大导致重渲染  
  - 应对：虚拟列表 + 富文本切分缓存
- 角色弹层卡片在 RN 上存在图片比例适配差异（局部裁剪/留白风险）  
  - 应对：统一素材比例基线；按卡片容器策略区分 `contain/cover`；回归 iOS/Android 双端视觉一致性
- 聊天组件二次定制成本  
...
| T-011 | 创作页（替换邂逅页） | doing | 已完成首版重构与主要交互（自由/固定模式、更多题材弹层、模式下拉、校验与通知）；当前进行设计稿像素级对齐与细节收敛 |
...
2. 当前优先顺序：
   - 推进 `T-011`：完成创作页剩余视觉细节与交互收口（保持中英文一致）
