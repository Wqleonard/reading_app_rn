...
- 当前进行中的细节优化（持续并入 Phase 5）：
  - 分支选择模块已按 Figma `selection-area` 对齐（标题/选项/custom-input）
  - 高光时刻标注样式已对齐（底色 `#005DFF` 4%）
  - 高光背景图展示仍待优化：当前存在“仅显示局部内容”问题，需继续校准图片容器比例与 `resizeMode` 策略
- 验收：
  - 完成一次完整阅读链路（含分支选择与回退）
...
| T-006 | 阅读器核心功能 | doing | 已完成连续渲染/选择跳转/进度恢复与回写/设置面板（滑轨/主题联动）持久化、亮度蒙层、高光图点击弹层+收录设背景（含本地资源迁移）、分支图面板自绘与回滚规则对齐、角色弹层（底部一体化容器+横向角色卡+解锁态+点击规则）对齐；分支选择模块已按 Figma 对齐，当前继续优化高光背景图展示完整性（避免只显示局部） |
| T-007 | 聊天与 AI | doing | 下一优先：重构 `app/chat/[id].tsx` 至 Flutter `character_chat` 对齐版本（UI/交互/安全区），随后接消息状态机与历史持久化 |
...
2. 当前优先顺序：
   - 先继续 `T-006`：优化阅读器高光背景图展示（完整显示、比例/对齐与 Flutter 保持一致）
   - 再推进 `T-007`：重构聊天页面（`app/chat/[id].tsx`）并补齐状态机
3. 每完成一个子任务，更新：
   - 任务状态表
   - 风险与问题记录
   - 已实现文件清单
...
> 1) 先优化阅读器高光背景图展示不全问题（`app/reader/[storyId].tsx`，保持与 Flutter 一致）；  
> 2) 然后重构聊天页 `app/chat/[id].tsx`，对齐 Flutter `reading_app/lib/pages/character_chat` 的 UI、交互和安全区；  
